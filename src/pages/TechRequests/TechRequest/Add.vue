...
<script>
import { Upload, Select, Option, Table, TableColumn } from 'element-ui';
...
export default {
    ...
    methods: {
        ...
        handleCurrentChange(val) {
            ...
            this.confirmEIN(val.doc.ein);
            
            //load information to form
            // this.form.organization.name = val.doc.name;
            // this.form.organization.legal_organization = val.doc.legal_organization;
            // this.form.organization.primary_contact = val.doc.primary_contact;
            // this.form.organization.secondary_contact = val.doc.secondary_contact;
            // this.form.organization.organization_type = val.doc.organization_type;
            // this.form.organization.website = val.doc.website;
            // this.form.organization.ein = val.doc.ein;
            // this.form.organization.acc_file = val.doc.acc_file;
        },
        confirmEIN(ein) {
            console.log(this);
            // this.$prompt("Enter organization's EIN to confirm your oganization", {
            ...
            //         this.$message({
            //             type: 'success',
            //             message: 'Added organization info succesfully!'
            //         });
            //     }).catch(() => {
            //         this.$message({
            //             type: 'danger',
        ...
        addMedia(link){
            if(link){
                this.form.organization.social_media.push({
        ...
        async addTechRequest(){
            ...
            if(!isValidForm){
                ...
                if(this.form.organization.organization_type !== ''){
                    var d = new Date();
                    this.form.organization.created_at.push(
                        d.getFullYear(),
                        d.getMonth(),
                        d.getDate(),
                        d.getHours(),
                    ...
                    this.form.organization.created_by = this.userSession.userCtx;
                    ...
                    this.$store.dispatch('addTechRequest', this.form.organization);
                }
                else{
                    this.$notify({
                        message: '<h6>Make sure to select a type</h6>',
                        type: 'warning'
                    });
                }
